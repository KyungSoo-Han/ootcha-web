...
function SearchModal(param, sub_param) {
    ...
    if(param =='destination' && document.getElementById('customerCd').value == '')
    {
        alert('화주사를 선택해야합니다.');
        return;
    }
...
function selectedModal() {
    ...
    if (document.getElementById('modal_param').value == 'customer') {
        ...
        document.getElementById('destinationCd').value = modal_dataProvider.getValue(modal_current.dataRow, 'code');
        document.getElementById('destinationNm').value = modal_dataProvider.getValue(modal_current.dataRow, 'name');
